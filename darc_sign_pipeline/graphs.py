...
#%% make a dataframe from each of sig type to produce the combined signature
# =============================================================================

def load_data(snv_counts_path, indel_counts_path, cnv_counts_path):
	df_snv = pd.read_csv(snv_counts_path, error_bad_lines=False)
	df_snv = df_snv[snv_categories]
	df_snv["sample"] = df_snv["sample"].astype(str)
	
	df_indel = pd.read_csv(indel_counts_path, error_bad_lines=False)
	df_indel = df_indel[indel_categories]
	df_indel["sample"] = df_indel["sample"].astype(str)
	
	df_cnv = pd.read_csv(cnv_counts_path, error_bad_lines=False)
...
#%% plot average naive tns signature for cohort or subset of cohort
# =============================================================================
...
def SNV_naive_signature(df_sig_count):
	...
	fig, ax = plt.subplots(nrows=1, ncols=6, sharey='all', figsize=(5, 1.0))
	...
	ax[0].set_ylim(0,normalized_sigs["amount"].max())
...
def INDEL_naive_signature(df_sig_count):
	...
	sns.set()
	sns.set_style(style="whitegrid")
	fig, ax = plt.subplots(nrows=1, ncols=16, sharey='all', figsize=(5, 1.0), gridspec_kw={'width_ratios':[6,6,6,6,6,6,6,6,6,6,6,6,1,2,3,5]})
	#plt.subplots_adjust(wspace=0.0)
...
def CN_naive_signature(df_sig_count):
	...
	sns.set()
	sns.set_style(style="whitegrid")
	fig, ax = plt.subplots(nrows=1, ncols=6, sharey="all", figsize=(5, 1.0), gridspec_kw={"width_ratios":[4,9,8,6,11,7]})
	sns.set_style(style="whitegrid")
