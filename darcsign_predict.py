...
def main():
	...
	parser.add_argument("-m", "--maf", help="Path to input maf file", dest="maf_path", type=str, required=True)
	parser.add_argument("-s", "--seg", help="Path to input segments file generated by sequenza. Unless otherwise named, sequenza names this file with suffix _segments.txt", dest="seg_path", type=str, required=True)
	parser.add_argument("-od", "--output_dir", help="Output directory for all naive signature context vectors and associated graphs. Often this directory is named the same as the sample name eg /Path/to/output/samplename" , dest="out_path", type=str, required=True)
	parser.add_argument("-sn", "--sample_name", help="The name of the sample. This name will be used in resulting output tables and figures. It will also be used in the naming of all ouptut files" , dest="sn", type=str, required=True)
	parser.add_argument("-md", "--model_directory", help="directory containing the 3 dna repair models" , dest="model_path", type=str, required=False)
